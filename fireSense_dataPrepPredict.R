defineModule(sim, list(
  ...
  reqdPkgs = list("data.table",
                  "SpaDES.core@development (>= 2.0.2.9012)",
...
.inputObjects <- function(sim) {
  ...
  if (!suppliedElsewhere("studyArea", sim)) {
   sim$studyArea <- LandR::randomStudyArea(size = 10000*6.25*500)
  }
  ...
  if (!suppliedElsewhere("flammableRTM", sim)){
    ...
    rstLCC <- project(rstLCC, crs(rstLCC), res = c(250, 250), method = "near")
  ...
  if (!suppliedElsewhere("pixelGroupMap", sim)) {
    warning("pixelGroupMap is missing. Generating random noise in place")
    sim$pixelGroupMap <- rast(sim$flammableRTM)
    sim$pixelGroupMap[sim$flammableRTM[] == 1] <- sample(1:sum(sim$flammableRTM[], na.rm = TRUE),
                                                          size = sum(sim$flammableRTM[], na.rm = TRUE),
                                                          replace = FALSE)
  }

  if (!suppliedElsewhere("projectedClimateRasters", sim)) {
    warning("projectedClimateRasters not supplied. Generating random noise in place")
    MDC <- rast(sim$flammableRTM, nlyrs = length(start(sim):end(sim)))
    MDC[] <- runif(ncell(MDC), min = 10, max = 200)
    names(MDC) <- paste0("year", start(sim):end(sim))
    fooName <- file.path(tempdir(), "MDCfoo.tif")
    writeRaster(MDC, fooName)
    sim$projectedClimateRasters <- list("MDC" = rast(fooName))
  }
